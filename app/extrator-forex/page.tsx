// frontend/app/extrator-forex/page.tsx (VERSÃO FINAL CORRIGIDA)

'use client';

import { useState, useEffect } from 'react'; // 1. Importa o useEffect
...
const AV_INTERVALS = ["1min", "5min", "15min", "30min", "60min", "D"];

export default function ExtratorForexPage() {
  const [symbols, setSymbols] = useState('EURUSD,USDBRL');
  const [interval, setInterval] = useState('D');
  
  // 2. Inicia os estados de data como strings vazias
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  
  const [isLoading, setIsLoading] = useState(false);

  // 3. Usa o useEffect para definir os valores iniciais APENAS no cliente
  useEffect(() => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      toast.success("Extração Iniciada!", {
        description: `A extração da Alpha Vantage começou. Verifique a Central de Relatórios em breve.`,
  ...
  return (
    <main className="bg-background min-h-screen flex flex-col items-center justify-center p-4">
      <Card className="w-full max-w-2xl">
        <CardHeader>
          <CardTitle className="text-2xl">Extrator de Dados (Alpha Vantage)</CardTitle>
          <CardDescription>
            Inicia a geração de um arquivo .zip com dados de Forex da Alpha Vantage.
            <span className="font-semibold text-destructive block mt-2">Nota: Dados intraday são limitados na API gratuita. Prefira o timeframe &apos;D&apos; (Diário).</span>
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Select value={interval} onValueChange={setInterval}>
              ...
              <SelectContent>
                {AV_INTERVALS.map(iv => <SelectItem key={iv} value={iv}>{iv}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
